// https://github.com/raygard/wak/tree/main
// Do this to build:
// gcc -Os -funsigned-char -std=c99 -Wall -Wextra -W -Wpointer-arith
//  -Wstrict-prototypes -pedantic  -D_POSIX_C_SOURCE=200809L ./programs/awk.c -static -s -o wak -lm

// Other compilers might work, as long as you link libm.
...
#define ZVINIT(flags, num, ptr)           \
    {                                     \
        (flags), (double)(num), { (ptr) } \
    }
...
static char *keywords =
    " in        BEGIN     END       if        else      "
    "while     for       do        break     continue  exit      function  "
    "return    next      nextfile  delete    print     printf    getline   ";

static char *builtins =
    " atan2     cos       sin       exp       log       "
...
static int zstring_hash(struct zstring *s) {  // djb2 -- small, fast, good enough for this
    unsigned h = 5381;
...
static void scan_opt_div(int div_op_allowed_here) {
    ...
    do ascan_opt_div(div_op_allowed_here);
    while (TT.scs->tok == tkerr);
}
...
static void scan(void) {
    ...
    if (TT.prevtok && strchr(div_preceders, TT.prevtok))
        scan_opt_div(1);
    else
        scan_opt_div(0);
...
static int getlbp(int tok) {
    ...
    return (0 <= tok && tok <= tkin) ? lbp_table[tok] :
               ...
               (tkgetline <= tok && tok <= tksubstr) ? 240
                                                     : 0;  // FIXME 240 is temp?
}
...
static void complain(int tk) {
    ...
    if (tksemi <= tk && tk <= tkpipe) {
        ...
        if (tk < tkatan2)
            memmove(tkstr, keywords + 1 + 10 * (tk - tkin), 10);
        else
            memmove(tkstr, builtins + 1 + 10 * (tk - tkatan2), 10);
        *strchr(tkstr, ' ') = 0;
        XERR("syntax near '%s' -- '%s' expected\n", TT.tokstr, tkstr);
    } else
        XERR("syntax near '%s'\n", TT.tokstr);
...
static void skip_to(char *tklist) {
    do scan();
    while (!ISTOK(tkeof) && !strchr(tklist, CURTOK()));
    if (ISTOK(tkeof)) unexpected_eof();
}
...
static void function_call(void) {
    ...
    if (ISTOK(tkbuiltin)) {
        ...
        error_exit("bad function %s!", TT.tokstr);
    scan();
...
static void convert_push_to_reference(void) {
    if (ZCODE[TT.zcode_last - 1] == tkvar)
        ZCODE[TT.zcode_last - 1] = opvarref;
    else if (ZCODE[TT.zcode_last - 1] == opmap)
        ZCODE[TT.zcode_last - 1] = opmapref;
    else if (ZCODE[TT.zcode_last - 1] == tkfield)
        ZCODE[TT.zcode_last - 1] = opfldref;
    else
        error_exit("bad lvalue?");
}
    ...
    switch (tok) {
        ...
        case tkplus:
            ...
            if (tok == tknot)
                ...
                gencd(opnegate);                 // forces to number
...
static int expr(int rbp) {
    ...
    if (rbp == CALLED_BY_PRINT) {
        if (prim_st > 0 && strchr(printexprendsy, CURTOK()))
            return prim_st;
        else
            rbp = 0;
    }
...
static int valid_for_array_iteration(int first, int last) {
    return ZCODE[first] == tkvar && ZCODE[first + 2] == tkvar && ZCODE[first + 4] == tkin && ZCODE[first + 5] == opdrop && first + 5 == last;
}
...
static void stmt(void) {
    switch (CURTOK()) {
        ...
        case tkbreak:
            scan();
            if (TT.cgl.break_dest)
                gen2cd(tkbreak, TT.cgl.break_dest - TT.zcode_last - 3);
            else
                XERR("%s", "break not in a loop\n");
            break;
        ...
        case tkexit:
            ...
            if (strchr(exprstartsy, CURTOK())) {
                ...
                gen2cd(tknumber, make_literal_num_val(NO_EXIT_STATUS));
            gencd(tkexit);
            break;
...
static void rule(void) {
    ...
    switch (CURTOK()) {
        case tkbegin:
            scan();
            if (TT.cgl.last_begin)
                ZCODE[TT.cgl.last_begin] = TT.zcode_last - TT.cgl.last_begin;
            else
                TT.cgl.first_begin = TT.zcode_last + 1;
        ...
        case tkend:
            scan();
            if (TT.cgl.last_end)
                ZCODE[TT.cgl.last_end] = TT.zcode_last - TT.cgl.last_end;
            else
                TT.cgl.first_end = TT.zcode_last + 1;
        ...
        case tklbrace:
            if (TT.cgl.last_recrule)
                ...
                TT.cgl.first_recrule = TT.zcode_last + 1;
            action(tkdo);
            gen2cd(opjump, -1);
...
static struct zstring *num_to_zstring(double n, char *fmt) {
    int k;
    if (n == (long long)n)
        k = snprintf(TT.pbuf, PBUFSIZE, "%lld", (long long)n);
    else
        k = snprintf(TT.pbuf, PBUFSIZE, fmt, n);
...
static int splitter(void (*setter)(struct zmap *, int, char *, size_t), struct zmap *m, char *s, struct zvalue *zvfs) {
    ...
    while (*s) {
        ...
        if ((r = rx_find_FS(rx, s, &offs, &end, eflag)))
            offs = end = strlen(s);
        else {
            int k = strcspn(s, "\n");
...
static void rebuild_field0(void) {
    ...
    for (int i = 1; i <= nf; i++) {
        ...
        if (FIELD[i].vst) {
            if (i > 1)
                s = zstring_extend(s, FIELD[i].vst);
            else
                s = zstring_copy(s, FIELD[i].vst);
        }
    }
    FIELD[0].vst = s;
...
static struct zvalue *setup_lvalue(int ref_stack_ptr, int parmbase, int *field_num) {
    ...
    *field_num = -1;
    ...
    if (ref->flags & ZF_REF) {
        ...
        FATAL("assignment to bad lvalue");
    ...
    for (struct zfile *p = TT.zfiles; p; p = np) {
        ...
        if ((!p->is_std_file) && (!fn || !strcmp(fn, p->fn))) {
            ...
            *pp = p->next;
            ...
            pp = &p->next;  // only if not unlinking zfile
    }
                ...
                switch (nargsneeded) {
                    case 1:
                        if (fmtc == 's')
                            ...
                            fpvar(outfp, pfmt, (wint_t)n);
                        else if (strchr("di", fmtc))
                            fpvar(outfp, pfmt, (long)n);
                        else if (strchr("ouxX", fmtc))
                            fpvar(outfp, pfmt, (unsigned long)n);
                        else
                            fpvar(outfp, pfmt, n);
                        break;
                    case 2:
                        if (fmtc == 's')
                            fpvar(outfp, pfmt, cnt2, s);
                        else if (fmtc == 'c')
                            fpvar(outfp, pfmt, cnt2, (wint_t)n);
                        else if (strchr("di", fmtc))
                            fpvar(outfp, pfmt, cnt2, (long)n);
                        else if (strchr("ouxX", fmtc))
                            fpvar(outfp, pfmt, cnt2, (unsigned long)n);
                        else
                            fpvar(outfp, pfmt, cnt2, n);
                        break;
                    case 3:
                        if (fmtc == 's')
                            fpvar(outfp, pfmt, cnt1, cnt2, s);
                        else if (fmtc == 'c')
                            fpvar(outfp, pfmt, cnt1, cnt2, (wint_t)n);
                        else if (strchr("di", fmtc))
                            fpvar(outfp, pfmt, cnt1, cnt2, (long)n);
                        else if (strchr("ouxX", fmtc))
                            fpvar(outfp, pfmt, cnt1, cnt2, (unsigned long)n);
                        else
                            fpvar(outfp, pfmt, cnt1, cnt2, n);
                        break;
                }
                break;
            default:
...
static int awk_getline(int source, struct zfile *zfp, struct zvalue *v) {
    ...
    if (v) {
        ...
        k = is_stream ? getrec_f0_f(zfp) : getrec_f0();
    return k < 0 ? 0 : 1;
}
    ...
    while ((opcode = *ip++)) {
        switch (opcode) {
            ...
            case tkprintf:
                ...
                switch (outmode) {
                    case tkgt:
                        outfp = setup_file(1, "w");
                        break;  // file
                    case tkappend:
                        outfp = setup_file(1, "a");
                        break;  // file
                    case tkpipe:
                        outfp = setup_file(0, "w");
                        break;  // pipe
                    default:
                        nargs++;
                        break;
...
#define FUNCTION_NUM 0
            ...
            case tkgetline:
                ...
                // v is NULL or an lvalue ref
                if (zfp != badfile)
                    push_int_val(awk_getline(source, zfp, v));
                else
                    push_int_val(-1);
            ...
            case tkmatch:
                ...
                if (k)
                    STACK[RSTART].num = 0, STACK[RLENGTH].num = -1;
                else {
            ...
            case tksrand:
                ...
                if (nargs == 1) {
                    ...
                    push_int_val(seedrand(millinow()));
                break;
            case tkcos:
            case tksin:
            case tkexp:
